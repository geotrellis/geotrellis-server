...
class GeoTrellisRasterSourceE(
  ...
  def this(attributeStore: AttributeStore, dataPath: GeoTrellisPath) =
    this(
      attributeStore,
      dataPath,
      GeoTrellisRasterSourceE.getSourceLayersByName(attributeStore, dataPath.layerName, dataPath.bandCount.getOrElse(1)),
      None,
      None
    )
  ...
  lazy val reader = S3CollectionLayerReaderE(attributeStore)
  ...
  def read(extent: Extent, bands: Seq[Int]): Option[Raster[MultibandTile]] =
    GeoTrellisRasterSourceE.read(reader, layerId, layerMetadata, extent, bands, time).map(convertRaster)
  ...
  def reprojection(
    ...
    println(s"${targetCRS} != ${this.crs}: ${targetCRS != this.crs}")
    println(s"resampleTarget: ${resampleTarget}")

    if (targetCRS != this.crs) {
      val reprojectOptions                   = ResampleTarget.toReprojectOptions(this.gridExtent, resampleTarget, method)
      val (closestLayerId, targetGridExtent) =
        GeoTrellisReprojectRasterSourceE.getClosestSourceLayer(targetCRS, sourceLayers, reprojectOptions, strategy)
      new GeoTrellisReprojectRasterSourceE(
        attributeStore,
        dataPath,
        closestLayerId,
        sourceLayers,
      ...
      resampleTarget match {
        ...
        case resampleTarget =>
          ...
          val closestLayerId      =
            GeoTrellisRasterSourceE.getClosestResolution(sourceLayers.toList, resampledGridExtent.cellSize, strategy)(_.metadata.layout.cellSize).id
          new GeoTrellisResampleRasterSourceE(
            attributeStore,
            dataPath,
            closestLayerId,
            sourceLayers,
  ...
  def resample(resampleTarget: ResampleTarget, method: ResampleMethod, strategy: OverviewStrategy): RasterSource = {
    ...
    val closestLayerId      =
      GeoTrellisRasterSourceE.getClosestResolution(sourceLayers.toList, resampledGridExtent.cellSize, strategy)(_.metadata.layout.cellSize).id
    new GeoTrellisResampleRasterSourceE(attributeStore, dataPath, closestLayerId, sourceLayers, resampledGridExtent, method, time, targetCellType)
  }

  def convert(targetCellType: TargetCellType): RasterSource =
    new GeoTrellisRasterSourceE(attributeStore, dataPath, sourceLayers, Some(targetCellType), time)

  override def toString: String = s"GeoTrellisRasterSourceE($dataPath, $layerId)"
}

object GeoTrellisRasterSourceE {
  ...
  def getSourceLayersByName(attributeStore: AttributeStore, layerName: String, bandCount: Int): Stream[Layer] = {
    attributeStore.layerIds
      ...
      .toStream
      . // We will be lazy about fetching higher zoom levels
      map { id =>
  ...
  def readTiles(
    ...
    println(s"readTile::reader: ${reader.getClass.toString}")
  ...
  def apply(dataPath: GeoTrellisPath): GeoTrellisRasterSourceE = GeoTrellisRasterSourceE(dataPath, None, None)

  def apply(dataPath: GeoTrellisPath, time: Option[ZonedDateTime]): GeoTrellisRasterSourceE = GeoTrellisRasterSourceE(dataPath, time, None)

  def apply(dataPath: GeoTrellisPath, time: Option[ZonedDateTime], targetCellType: Option[TargetCellType]): GeoTrellisRasterSourceE = {
