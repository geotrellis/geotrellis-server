...
class GeoTrellisReprojectRasterSourceE(
  ...
  def read(extent: Extent, bands: Seq[Int]): Option[Raster[MultibandTile]] = {
    ...
    for {
      ...
      raster            <- GeoTrellisRasterSourceE.readIntersecting(reader, layerId, sourceLayer.metadata, sourceExtent, bands, time)
    } yield {
  ...
  def reprojection(
    ...
    if (targetCRS == sourceLayer.metadata.crs) {
      ...
      val closestLayer        =
        GeoTrellisRasterSourceE.getClosestResolution(sourceLayers, resampledGridExtent.cellSize, strategy)(_.metadata.layout.cellSize)
      ...
      // Pick new layer ID
      val (_, gridExtent) =
        GeoTrellisReprojectRasterSourceE
  ...
  def resample(resampleTarget: ResampleTarget, method: ResampleMethod, strategy: OverviewStrategy): RasterSource = {
    ...
    val (closestLayerId, newGridExtent) = GeoTrellisReprojectRasterSourceE.getClosestSourceLayer(crs, sourceLayers, newReprojectOptions, strategy)
    new GeoTrellisReprojectRasterSourceE(
      attributeStore,
      dataPath,
      closestLayerId,
  ...
  def convert(targetCellType: TargetCellType): RasterSource =
    new GeoTrellisReprojectRasterSourceE(
      ...
      resampleMethod,
      time = time,
      targetCellType = Some(targetCellType)
    )

  override def toString: String = s"GeoTrellisReprojectRasterSourceE(${dataPath.value},$layerId,$crs,$gridExtent,$resampleMethod)"
}

object GeoTrellisReprojectRasterSourceE {
  ...
  def getClosestSourceLayer(
    ...
    if (options.targetRasterExtent.isDefined) {
      ...
      val sourceLayer                  = GeoTrellisRasterSourceE.getClosestResolution(sourcePyramid, sourceGrid.cellSize, strategy)(_.metadata.layout.cellSize)
      ...
      val sourceLayer: Layer = {
        ...
        GeoTrellisRasterSourceE.getClosestResolution(sourcePyramid, aproximateSourceCellSize, strategy)(_.metadata.layout.cellSize)
      }
      ...
      val sourceLayer                             = GeoTrellisRasterSourceE.getClosestResolution(sourcePyramid, aproximateSourceCellSize, strategy)(_.metadata.layout.cellSize)
      val gridExtent: GridExtent[Long]            = ReprojectRasterExtent(sourceLayer.gridExtent, sourceLayer.metadata.crs, targetCRS, options)
      (sourceLayer.id, gridExtent)
