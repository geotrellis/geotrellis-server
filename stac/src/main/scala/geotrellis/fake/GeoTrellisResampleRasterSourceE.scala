...
/** RasterSource that resamples on read from underlying GeoTrellis layer.
  ...
  */
class GeoTrellisResampleRasterSourceE(
  ...
  def read(extent: Extent, bands: Seq[Int]): Option[Raster[MultibandTile]] = {
    ...
    def msg        =
      s"\u001b[32mread($extent)\u001b[0m = ${dataPath.toString} ${sourceLayer.id} ${sourceLayer.metadata.cellSize} @ ${sourceLayer.metadata.crs} TO $cellSize -- reading ${tileBounds.size} tiles"
    ...
    GeoTrellisRasterSource
      .readIntersecting(reader, layerId, sourceLayer.metadata, extent, bands, time)
      .map { raster =>
  ...
  def reprojection(
    ...
    val (_, gridExtent)  = GeoTrellisReprojectRasterSourceE.getClosestSourceLayer(targetCRS, sourceLayers, reprojectOptions, strategy)
  ...
  /** Resample underlying RasterSource to new grid extent
    ...
    */
  def resample(resampleTarget: ResampleTarget, method: ResampleMethod, strategy: OverviewStrategy): GeoTrellisResampleRasterSourceE = {
    ...
    new GeoTrellisResampleRasterSourceE(attributeStore, dataPath, closestLayer.id, sourceLayers, resampledGridExtent, method, time, targetCellType)
  }

  def convert(targetCellType: TargetCellType): GeoTrellisResampleRasterSourceE =
    new GeoTrellisResampleRasterSourceE(attributeStore, dataPath, layerId, sourceLayers, gridExtent, resampleMethod, time, Some(targetCellType))
  ...
  override def toString: String =
    s"GeoTrellisResampleRasterSourceE(${dataPath.toString},$layerId,$gridExtent,$resampleMethod)"
}
