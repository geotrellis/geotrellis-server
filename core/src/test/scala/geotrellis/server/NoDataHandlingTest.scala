...
class NoDataHandlingTest
    extends FunSuite
    with Matchers
    with TileAsSourceImplicits {
  ...
  test(
    ...
    val t1 = IntUserDefinedNoDataArrayTile(
      (1 to 100).toArray,
      10,
      10,
      IntUserDefinedNoDataCellType(1)
    )
    val t2 = IntUserDefinedNoDataArrayTile(
      (1 to 100).toArray,
      10,
      10,
      IntUserDefinedNoDataCellType(1)
  ...
  test("NODATA should be respected - different source celltypes") {
    ...
    val t2 = DoubleUserDefinedNoDataArrayTile(
      (1 to 100).map(_.toDouble).toArray,
      10,
      10,
      DoubleUserDefinedNoDataCellType(2.0)
    )
    val paramMap = Map("t1" -> t1, "t2" -> t2)
    // We'll sample such that the bottom row (from 56 to 64) are excised from the result
    val res = eval(paramMap, 0, 0, 0).unsafeRunSync
