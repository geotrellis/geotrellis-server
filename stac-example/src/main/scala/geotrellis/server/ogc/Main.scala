      ...
      main = {
        ...
        val configPathOpt = Opts
          .option[String](
            ...
            short = "c",
            help =
              "Full path to a HOCON configuration file (https://github.com/lightbend/config/blob/master/HOCON.md)"
          )
          .orNone
        ...
        (publicUrlReq, interfaceOpt, portOpt, configPathOpt).mapN {
          (publicUrl, interface, port, configPath) =>
            {
              // logger.info(
              //   ansi"%green{Locally binding services to ${interface}:${port}}"
              // )
              // logger.info(ansi"%green{Advertising services at ${publicUrl}}")
              // configPath match {
              //   case Some(path) =>
              //     logger.info(
              //       ansi"%green{Layer and style configurations loaded from ${path}.}"
              //     )
              //   case None =>
              //     logger.info(
              //       ansi"%red{Warning}: No configuration path provided. Loading defaults."
              //     )
              // }
              ...
              val commonMiddleware: HttpMiddleware[IO] = {
