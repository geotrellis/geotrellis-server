    ...
    ("Red" :: "Green" :: "Blue" :: Nil).map { band =>
      val clamp =
        (params.validatedOptionalParamDouble(s"clampMin$band").toOption.flatten,
         params.validatedOptionalParamDouble(s"clampMax$band").toOption.flatten).mapN(Clamp)
      ...
      val normalize =
        (params.validatedOptionalParamDouble(s"normalizeOldMin$band").toOption.flatten,
         ...
         params.validatedOptionalParamDouble(s"normalizeNewMax$band").toOption.flatten).mapN(Normalize)

      val rescale =
        (params.validatedOptionalParamDouble(s"rescaleNewMin$band").toOption.flatten,
         params.validatedOptionalParamDouble(s"rescaleNewMax$band").toOption.flatten).mapN(Rescale)
